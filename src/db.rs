...
pub async fn run_v14_migrations(pool: &sqlx::PgPool) -> anyhow::Result<()> {
    tracing::info!("Running v14 migrations (Gap 7: refresh tokens)...");
    sqlx::query("
        CREATE TABLE IF NOT EXISTS frank_refresh_tokens (
            id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            user_id     UUID NOT NULL REFERENCES frankos_users(id) ON DELETE CASCADE,
    ...
    ").execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_refresh_tokens_user ON frank_refresh_tokens(user_id);").execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_refresh_tokens_hash ON frank_refresh_tokens(token_hash);").execute(pool).await?;
